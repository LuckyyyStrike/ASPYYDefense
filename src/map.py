...
class Map:
    ...
    def __init__(self, map_description: MapDescription, max_width: int, max_height: int):
    ...
    def create_row(
        ...
        for column in range(padding_count_infront):
            columns.append(Entity(column_index, row_index, padding_character))
            column_index += 1
        for content_character in content:
            columns.append(Entity(column_index, row_index, content_character))
            column_index += 1
        for column in range(padding_count_behind):
            columns.append(Entity(column_index, row_index, padding_character))
            column_index = +1
        columns.append(Entity(column_index, row_index, "I"))
        return columns
